...
export function matchMethodCall(
  ...
  return matchDirectClassMethod(ref, objectOrClass!, methodName!, context)
    ?? matchCapitalizedReceiverMethod(ref, objectOrClass!, methodName!, context)
    ?? matchByMethodNameSearch(ref, objectOrClass!, methodName!, context);
}

function matchDirectClassMethod(
  ref: UnresolvedRef,
  ...
  for (const classNode of classCandidates) {
    ...
    if (methodNode) {
      return { original: ref, targetNodeId: methodNode.id, confidence: 0.85, resolvedBy: 'qualified-name' };
    }
  }
  return null;
}

function matchCapitalizedReceiverMethod(
  ref: UnresolvedRef,
  objectOrClass: string,
  methodName: string,
  context: ResolutionContext
  ...
  for (const classNode of fuzzyClassCandidates) {
    ...
    if (methodNode) {
      return { original: ref, targetNodeId: methodNode.id, confidence: 0.8, resolvedBy: 'instance-method' };
    }
  }
  return null;
}

function matchByMethodNameSearch(
  ref: UnresolvedRef,
  objectOrClass: string,
  methodName: string,
  context: ResolutionContext
  ...
  if (targetMethods.length === 1 && targetMethods[0]!.language === ref.language) {
    return { original: ref, targetNodeId: targetMethods[0]!.id, confidence: 0.7, resolvedBy: 'instance-method' };
  }
  ...
  if (targetMethods.length > 1) {
    ...
    for (const method of targetMethods) {
      ...
      if (score > bestScore) { bestScore = score; bestMatch = method; }
    }
    if (bestMatch && bestScore >= 2) {
      return { original: ref, targetNodeId: bestMatch.id, confidence: 0.65, resolvedBy: 'instance-method' };
    }
  }
